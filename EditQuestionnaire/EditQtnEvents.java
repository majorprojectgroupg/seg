...
class EditQtnEvents implements SelectionListener, KeyListener, ModifyListener
{
	...
	public void widgetSelected(SelectionEvent e)
	{
		...
		// -- 'editAnsBtn' -- Editing answers in the list. -- //
			...
			// -- 'openMenuItem' code -- This section works fine :) //
			
			if(e.getSource() == EditQ.openMenuItem)
			{
				// Opens the dialog.
				EditQ.loadQtnDialog.open();
				
				EditQ.questionnaireName = EditQ.loadQtnDialog.getFileName();
				...
				{					
						...
						// Now, we retrieve the questionnaire ID.
						// Drop the .json in the String.
						EditQ.questionnaireName = EditQ.questionnaireName.replace(".json","");
						// Store the ID as int so we can play with it later ;)
						EditQ.questionnaireID = Integer.parseInt(EditQ.questionnaireName.substring(14));		
						// Debug purposes.
						System.out.println("The selected questionnaire's ID is : " + EditQ.questionnaireID);
						...
						try
						{
							...
							while(rs.next())
							{
								...
								arrayQuestionNames.add(rs.getString("question_name"));
								// Retrieve the Question type
								arrayQuestionType.add(rs.getString("question_type"));
							...
							Qtns = arrayQuestionNames.toArray(Qtns);
